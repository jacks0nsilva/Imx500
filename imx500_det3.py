...
def atribuir_id_por_posicao(box):
    cx, cy = centro_box(box)
    largura, altura = 640, 640  # Resolução fixa
    if cx < largura / 2 and cy < altura / 2:
        return 0  # superior esquerdo
    elif cx < largura / 2 and cy >= altura / 2:
        return 1  # inferior esquerdo
    elif cx >= largura / 2 and cy < altura / 2:
        return 2  # superior direito
    else:
...
def registrar_quatro_apagados(deteccoes):
    ...
    for d in usados:
        id_q = atribuir_id_por_posicao(d.box)
