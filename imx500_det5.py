...
QUEIMADORES_REFERENCIA = {
    0: {"nome": "superior_esquerdo",  "cx": 0, "cy": 0},
    1: {"nome": "inferior_esquerdo",  "cx": 0, "cy": 0},
    2: {"nome": "superior_direito",   "cx": 0, "cy": 0},
    3: {"nome": "inferior_direito",   "cx": 0, "cy": 0},
}

TOLERANCIA = 50  # ±50 pixels em X e Y
...
def dentro_da_tolerancia(cx, cy, ref_x, ref_y, tol):
    return (
        abs(cx - ref_x) <= tol and
        abs(cy - ref_y) <= tol
    )


def atribuir_id_por_regiao(box):
    cx, cy = centro_box(box)

    for id_q, ref in QUEIMADORES_REFERENCIA.items():
        if dentro_da_tolerancia(cx, cy, ref["cx"], ref["cy"], TOLERANCIA):
            return id_q

    return None  # Não encontrou região válida
...
def registrar_quatro_apagados(deteccoes):
    ...
    for d in usados:
        id_q = atribuir_id_por_regiao(d.box)
        if id_q is None:
            continue  # Ignora se não conseguiu atribuir ID
